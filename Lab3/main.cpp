...
public:
	...
	BilateralParameters(Mat in, int sz, double sigma_color, double sigma_space)
	{
		filter = BilateralFilter(in, sz, sigma_color, sigma_space);
	}
...
int main(int argc, char** argv){
	...
	//Loading and displaying the image____________________________________________________________________
	cout << "Loading original image... Done\n\n" << endl;
	...
	//Calculating histograms______________________________________________________________________________
	...
	//Equalizing each channel_____________________________________________________________________________
	...
	//Working on a different color space__________________________________________________________________

	cout << "Now working on \"lab\" color space. \n" << endl;

	///Calculating histograms_____________________________________________________________________________
	...
	///Equalizing only one channel________________________________________________________________________
	...
	cout << "Showing image windows with trackbars for filter parameters: \n";


	/// Median____________________________________________________________________________________________

	MedianParameters medianParameters(edit_result, 0);
	...
	GaussianParameters gaussianParameters(edit_result, 0,0);
	...
	createTrackbar("sigma", "Gaussian Filter window", &(gaussianParameters.sigma), 200, gaussian_onTrackbar, (void*)(&gaussianParameters));
	...
	BilateralParameters bilateralParameters(edit_result,0,0,0);
	...
	createTrackbar("sigmaColor", "Bilateral Filter window", &(bilateralParameters.s_color), 200, bilateral_onTrackbar, (void*)(&bilateralParameters));
	createTrackbar("sigmaSpace", "Bilateral Filter window", &(bilateralParameters.s_space), 200, bilateral_onTrackbar, (void*)(&bilateralParameters));
